...
public class CalculateDevelopTime extends HttpServlet implements Servlet {
	/**
	 * 
	 */
	...
	protected void doPost(HttpServletRequest request,
		...
		if (jiraissuelist.size() == 0) {
			...
			// “开发人天”按钮：
			String sumtimeworked1 = connall.getDeveloper_worklog(batch,
					createddate, projectid);
			insertall.getDeveloper_worklog(sumtimeworked1, batch, createddate,
					projectid);
			// 获取 "测试人天"的字段成json格式
	...
	public String getqareportJson(String createddate) {
		...
		try {
			...
			String sql = " select projid,sumworked,batch,createddate "
					+ " from developer_worklog where createddate='"
					+ createddate + "' ";
			rs = st.executeQuery(sql);// 返回查询结果
			String SQL = "  select count(*) as sum  from developer_worklog where createddate='"
					+ createddate + "' ";
			rs1 = st1.executeQuery(SQL);
			rs1.next();
			int count = rs1.getInt("sum");
			// System.out.println("chaxungeshu"+count);
			int i = 0;
